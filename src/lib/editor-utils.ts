...
import { getDiscordConnectionUrl } from "./discord-connection";
import { getNotionConnection, getNotionDatabase } from "./notion-connection";
import { getSlackConnection, listBotChannels } from "./slack-connection";
...
export const onConnections = async (
  ...
  if (editorState.editor.selectedNode.data.title == "Discord") {
    const connectionUrl = await getDiscordConnectionUrl();

    if (connectionUrl) {
      nodeConnection.setDiscordNode({
        webhookURL: connectionUrl.url,
        content: "",
        webhookName: connectionUrl.name,
        guildName: connectionUrl.guildName,
      });
    }
  }
